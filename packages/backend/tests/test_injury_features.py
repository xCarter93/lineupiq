...
def test_injury_severity_encoding():
    ...
    assert result.filter(pl.col("player_id") == "P1")["injury_severity"][0] == 1.0  # Out
    assert (
        result.filter(pl.col("player_id") == "P2")["injury_severity"][0] == 0.75
    )  # Doubtful
    assert (
        result.filter(pl.col("player_id") == "P3")["injury_severity"][0] == 0.5
    )  # Questionable
    assert (
        result.filter(pl.col("player_id") == "P4")["injury_severity"][0] == 0.25
    )  # Probable
    assert (
        result.filter(pl.col("player_id") == "P5")["injury_severity"][0] == 0.0
    )  # None status
    assert (
        result.filter(pl.col("player_id") == "P6")["injury_severity"][0] == 0.0
    )  # Not in injury data
...
def test_on_injury_report_flag():
    ...
    assert result.filter(pl.col("player_id") == "P1")["on_injury_report"][0] == 1
    assert result.filter(pl.col("player_id") == "P2")["on_injury_report"][0] == 1

    # P3 not on injury report
    assert result.filter(pl.col("player_id") == "P3")["on_injury_report"][0] == 0
...
def test_join_on_player_week():
    ...
    # P1 week 1: no injury
    assert (
        result.filter((pl.col("player_id") == "P1") & (pl.col("week") == 1))[
            "injury_severity"
        ][0]
        == 0.0
    )
    # P1 week 2: Out (1.0)
    assert (
        result.filter((pl.col("player_id") == "P1") & (pl.col("week") == 2))[
            "injury_severity"
        ][0]
        == 1.0
    )
    ...
    assert (
        result.filter((pl.col("player_id") == "P2") & (pl.col("week") == 1))[
            "injury_severity"
        ][0]
        == 0.5
    )
    # P2 week 2: no injury
    assert (
        result.filter((pl.col("player_id") == "P2") & (pl.col("week") == 2))[
            "injury_severity"
        ][0]
        == 0.0
    )
...
def test_injury_features_without_date_modified():
    ...
    assert result.filter(pl.col("player_id") == "P1")["injury_severity"][0] == 1.0
    assert result.filter(pl.col("player_id") == "P2")["injury_severity"][0] == 0.5
    assert result.filter(pl.col("player_id") == "P1")["on_injury_report"][0] == 1
    assert result.filter(pl.col("player_id") == "P2")["on_injury_report"][0] == 1
