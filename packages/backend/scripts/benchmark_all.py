"""
Comprehensive benchmark script for all skill position stats.

Runs benchmark_ensemble_strategies on all QB/RB/WR/TE stats and
generates BENCHMARK_RESULTS.md with full analysis.
"""
...
def main() -> None:
    """Run comprehensive benchmark on all skill position stats."""
    logger.info("Starting comprehensive benchmark for all skill position stats")
    ...
    logger.info("Generating BENCHMARK_RESULTS.md")
...
def generate_results_markdown(all_results: list[dict]) -> None:
    """Generate BENCHMARK_RESULTS.md with comprehensive analysis."""
    output_path = Path(__file__).parent.parent.parent.parent / ".planning" / "phases" / "19-ensemble-models" / "BENCHMARK_RESULTS.md"
    ...
    # Generate markdown
    content = f"""# Ensemble Benchmark Results
